...
export const createIfthenpayCheckout = webMethod(Permissions.Anyone, (paymentDetails) => {
    ...
    const gatewayKey = "Gateway key"; // Obtain from Ifthenpay
    const baseUrl = "https://gateway.ifthenpay.com/";

    // Construct the Simple Checkout URL with required parameters
    let paymentUrl = `${baseUrl}?token=${gatewayKey}&id=${paymentDetails.orderId}&amount=${paymentDetails.amount}&description=${encodeURIComponent(paymentDetails.description)}&expire=${paymentDetails.expire}&lang=${paymentDetails.lang}&selected_method=${paymentDetails.selectedMethod}&accounts=${paymentDetails.accounts}`;
